...
//        fArgumentIndex = index;
     ...
     */
    @Override
    public int getAttributeQuark(int startQuark, String scenarioName, String activeState) {
        return getAttributeQuark(null, startQuark, scenarioName, activeState);
    }
     ...
     */
    @Override
    public int getAttributeQuark(@Nullable ITmfEvent event, int startQuark, String scenarioName, String activeState) {
        ITmfStateSystem ss = getStateSystem();

//        String name = fName;
//        if (fArgumentIndex >= 0) {
//            if (fArgumentIndex < args.length) {
//                name = args[fArgumentIndex];
//            } else {
//                throw new IllegalArgumentException("Invalid argument : argument " + fArgumentIndex + "does not exist. The maximum number possible is" + args.length); //$NON-NLS-1$ //$NON-NLS-2$
//            }
//        }
        String name = nullToEmptyString(fName).equals("#scenarioName") ? scenarioName : nullToEmptyString(fName).equals("#currentState") ? activeState : fName; //$NON-NLS-1$ //$NON-NLS-2$
        ...
        try {
            ...
            case QUERY: {
                ...
                for (ITmfXmlStateAttribute attrib : fQueryList) {
                    quarkQuery = attrib.getAttributeQuark(event, quarkQuery, scenarioName, activeState);
                    if (quarkQuery == IXmlStateSystemContainer.ERROR_QUARK) {
                        break;
                    }
                }
            ...
            case LOCATION: {
                ...
                for (TmfXmlLocation location : fContainer.getLocations()) {
                    if (location.getId().equals(idLocation)) {
                        quark = location.getLocationQuark(event, quark, scenarioName, activeState);
                        if (quark == IXmlStateSystemContainer.ERROR_QUARK) {
                            break;
                        }
                    }
