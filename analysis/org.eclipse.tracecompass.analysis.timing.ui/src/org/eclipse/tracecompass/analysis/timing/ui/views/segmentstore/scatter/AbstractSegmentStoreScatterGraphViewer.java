...
public abstract class AbstractSegmentStoreScatterGraphViewer extends TmfCommonXLineChartViewer {
    ...
    /**
     * Listener to update the model with the segment store provider results
     * once its segment store is fully completed
     */
     ...
     * update the model if analysis is completed or run the analysis if not
     * completed
     *
     * @param provider
    ...
    public void setData(@Nullable ISegmentStoreProvider provider) {
        ...
        if (segStore != null) {
            ...
            stSegmentProvider(provider);
            return;
        }
        updateModel(null);
        provider.addListener(fListener);
        if( provider instanceof TmfAbstractAnalysisModule) {
            TmfAbstractAnalysisModule analysisModule = (TmfAbstractAnalysisModule) provider;
    ...
    private void stSegmentProvider(ISegmentStoreProvider provider) {
